            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <form method="POST" action="{{ route('payments.store') }}">
                        ...
                        <input class="appearance-none block w-1/3 bg-white text-gray-700 border border-solid border-gray-300 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white" id="grid-first-name" type="text" name="amount" placeholder="Amount of payment" value="{{ old('amount') }}">
                        {{ __('Money type') }}:
                        <div class="mb-3 w-1/3">
                            <select name="money_type_id" class="form-select appearance-none block w-full px-3 py-1.5 text-base font-normal text-gray-700 bg-white bg-clip-padding bg-no-repeat border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none" >
                                @foreach($money_types as $money_type)
                                    <option value="{{ $money_type->id }}"  @selected(old('payment_type') == $money_type->id)>{{ $money_type->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        {{ __('Payment type') }}:
